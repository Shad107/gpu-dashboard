...
- **Power-limit slider** — adjust GPU watts cap from the UI (with perf% estimation per card)
- **Clock-offset sliders** — undervolt or overclock with safe/moderate/aggressive/danger zones
- **GPU profiles per card** — RTX 3090, 3090 Ti, 4090, 5090 included, generic fallback,
  community can contribute more via PR
...
- `nvtop` is great in a TTY but no control, no alerting
...
| RTX 5090 | `rtx-5090.json` | ⚠ Based on public benchmarks |
...
The installer detects your environment (OS, NVIDIA driver, X server, Coolbits, sudoers)
and only proposes the modules that can work on your machine. **No silent sudo, no
auto-install of packages.** It tells you what it would do, you confirm.
...
│   ├── config.py              # .env layered config loader
│   ├── profile.py             # GPU profile load + match + schema validation
...
│       └── telegram_alerts.py # urllib stdlib, no `requests`
├── profiles/                  # JSON profiles + JSON Schema
└── tests/                     # pytest, 161 tests, no external services
```

## Optional features

Each feature can be enabled or disabled — `install.sh` only offers what your env supports.

| Module | Needs | What it adds |
|---|---|---|
| **power_limit** | sudoers wrapper installed | UI slider 100-350W (or per-card max), perf% live estimate |
...
Profiles for new cards are **the most valuable contribution**. See
