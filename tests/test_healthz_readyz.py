...
def test_readyz_strict_ecc_failing_returns_503(ctx):
    ...
    with patch.object(api, "_gpus_available", return_value=[{"index": 0}]), \
         patch.object(api, "handle_ecc_health", return_value=(200, fake_ecc)), \
         patch.object(api, "handle_drift_check", return_value=(200, {"last_drift": None})):
        code, body = api.handle_readyz(ctx, {"strict": "1"})
    assert code == 503
...
def test_readyz_strict_recent_drift_returns_503(ctx):
    ...
    with patch.object(api, "_gpus_available", return_value=[{"index": 0}]), \
         patch.object(api, "handle_ecc_health", return_value=(200, {"verdict_kind": "ok"})), \
         patch.object(api, "handle_drift_check", return_value=(200, fake_drift)):
        code, body = api.handle_readyz(ctx, {"strict": "1"})
    assert code == 503
