...
def test_emits_data_when_gpu_alive():
    ...
    with patch.object(api._core, "_gpus_available", return_value=fake_gpus), \
         patch.object(api._core, "_gpu_card_snapshot", return_value=fake_snap), \
         patch.object(api._core, "_per_fan_state", return_value=[]):
        code, body = api.handle_prometheus_metrics(_ctx())
    assert code == 200
