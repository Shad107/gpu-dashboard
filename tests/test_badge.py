...
def test_badge_gpu_temp_ok():
    """Temp < 70°C → green (#4c1)."""
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(temp=42)):
...
def test_badge_gpu_temp_warn():
    """70-80°C → yellow."""
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(temp=75)):
        code, svg = api.handle_badge(_ctx(), "gpu-temp")
...
def test_badge_gpu_temp_crit():
    """>= 80°C → red."""
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(temp=85)):
        code, svg = api.handle_badge(_ctx(), "gpu-temp")
...
def test_badge_power_now_includes_watts():
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(power=240.5)):
...
def test_badge_util_color_thresholds():
    ...
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(util=10)):
        _, svg = api.handle_badge(_ctx(), "util")
    assert "#4c1" in svg
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(util=70)):
        _, svg = api.handle_badge(_ctx(), "util")
    assert "#dfb317" in svg
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(util=95)):
        _, svg = api.handle_badge(_ctx(), "util")
    assert "#e05d44" in svg
...
def test_badge_top_model_shows_short_name():
    with patch.object(api._monolith, "_gpu_card_snapshot", return_value=_snap(name="NVIDIA GeForce RTX 3090")):
