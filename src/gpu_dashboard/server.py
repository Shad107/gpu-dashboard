    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if path == "/api/cpuset-v2-partition-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/tracing-instances-audit":
                # R&D #96.3 — per-instance ftrace buffer auditor
            ...
            if path == "/api/psi-irq-full-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/fs-quota-projid-audit":
                # R&D #98.2 — fs quota / projid / overlay-on-quota
            ...
            if path == "/api/umwait-control-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/split-lock-detect-audit":
                # R&D #99.2 — split-lock detection posture
            ...
            if path == "/api/bql-stall-counters-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/perf-sampling-limits-audit":
                # R&D #100.3 — perf/eBPF sampling limits
            ...
            if path == "/api/kfence-runtime-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/net-qdisc-default-audit":
                # R&D #101.2 — default network qdisc posture
            ...
            if path == "/api/bpf-jit-harden-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/kernel-oops-warn-counter-audit":
                # R&D #103.1 — kernel oops/warn counters since boot
                code, body = api.handle_kernel_oops_warn_counter_audit_status(ctx)
                self._send_json(code, body)
                return
            if path == "/api/ephemeral-port-range-audit":
                # R&D #103.2 — ephemeral TCP port window posture
            ...
            if path == "/api/nvidia-drm-params-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/overlay-module-params-audit":
                # R&D #108.2 — overlayfs driver tunables
            ...
            if path == "/api/dm-mod-params-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/cgroup-tree-limits-audit":
                # R&D #108.4 — cgroup.max.depth + descendants
            ...
            if path == "/api/acpi-boot-assets-audit":
                ...
                self._send_json(code, body)
                return
            if path == "/api/acpi-tables-inventory-audit":
                # R&D #109.3 — ACPI table set sanity
            ...
            if path == "/api/shadow-telemetry":
                ...
                self._send_json(code, body)
                return
            if path == "/api/health-strip":
                # F5.1 — aggregated curated audit verdicts
